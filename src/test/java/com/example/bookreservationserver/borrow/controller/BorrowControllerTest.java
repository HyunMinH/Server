...
class BorrowControllerTest {
    ...
    @Test
    @DisplayName("대여하기 성공")
    void testBookBorrow() throws Exception {
        //given
        final BorrowRequest borrowRequestDto = borrowRequestDto();
        doReturn(borrowResponseDto()).when(borrowService).borrowBook(any());
        ...
        final BorrowBookResponse borrowBookResponse = gson.fromJson(mvcResult.getResponse().getContentAsString(), BorrowBookResponse.class);
        assertEquals(borrowBookResponse.getState(), BorrowState.BORROWING);
    }
    ...
    @Test
    @DisplayName("모든 유저의 대여 중인 기록 가져오기")
    ...
    @Test
    @DisplayName("한사람의 모든 대여 기록 가져오기")
    ...
    @Test
    @DisplayName("한 사람의 대여중인 대여 기록 가져오기")
    void testGetMyBorrowing() throws Exception{
        //given
        doReturn(Collections.emptyList()).when(searchService).getMyBorrowings(any());
    ...
    @Test
    @DisplayName("한 사람의 연체된 대여 기록 가져오기")
